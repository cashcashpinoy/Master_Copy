...
class rsCarthelper {
	...
	var $_extra_field = null;
	var $_redhelper = null;
	var $_producthelper = null;
	var $_show_with_vat = 0;
...
//					if ($billingaddresses->requisition_number != "")
//					{
//						$billingdata=str_replace("{requisition_number}",$billingaddresses->requisition_number,$billingdata);
//						$billingdata=str_replace("{requisition_number_lbl}",JText::_('COM_REDSHOP_REQUISITION_NUMBER'),$billingdata);
//					}
...
//			$billingdata=str_replace("{requisition_number}","",$billingdata);
//			$billingdata=str_replace("{requisition_number_lbl}","",$billingdata);
		...
		{
			$billadd = '<table style="width:300px;background: none repeat scroll 0 0;">';
			if(isset($billingaddresses))
			{
				$extra_section = ($billingaddresses->is_company==1) ? 8 : 7;
				if ($billingaddresses->is_company == 1 && $billingaddresses->company_name != "")
				{
					$billadd .=  '<tr><td>'.JText::_('COM_REDSHOP_COMPANY_NAME').' </td><td> '.$billingaddresses->company_name.'</td></tr>';
				}
				if ($billingaddresses->firstname != "") {
					$billadd .= '<tr><td style="width:86px;">'.JText::_("COM_REDSHOP_FIRSTNAME" ) . ' </td><td> : ' . $billingaddresses->firstname.'</td></tr>' ;
				}
				if ($billingaddresses->lastname != "") {
					$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_LASTNAME" ) . ' </td><td> : ' . $billingaddresses->lastname . '</td></tr>';
				}
				if ($billingaddresses->address != "") {
					$billadd .=  '<tr><td><span style="display: block;">'.JText::_("COM_REDSHOP_ADDRESS" ) . ' </span></td><td> : ' . $billingaddresses->address . '</td></tr>';
				}
				$cname = $this->_order_functions->getCountryName( $billingaddresses->country_code );
				if ($cname != "")
				{
					$billadd .=  '<tr><td>'.JText::_("COM_REDSHOP_COUNTRY" ) . ' </td><td> : ' . JText::_($cname) . '</td></tr>';
				}
				$sname = $this->_order_functions->getStateName ( $billingaddresses->state_code, $billingaddresses->country_code );
				if ($sname != "") {
					$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_STATE" ).' </td><td> : '.$sname.'</td></tr>';
				}
				if ($billingaddresses->city != "") {
					$billadd .=  '<tr><td>'.JText::_("COM_REDSHOP_CITY" ) . '  </td><td> : ' . $billingaddresses->city . '</td></tr>';
				}
				if ($billingaddresses->zipcode != "") {
					$billadd .=  '<tr><td>'.JText::_("COM_REDSHOP_ZIP" ) . '  </td><td> : ' . $billingaddresses->zipcode . '</td></tr>';
				}

				//Birthday

				$birthday = $this->_extra_field->displayValueMobile($extra_section,6,$billingaddresses->users_info_id);

				if($birthday!=""){
					$billadd .= '<tr><td>Birthday<td> : ' .$this->_extra_field->displayValueMobile($extra_section,6,$billingaddresses->users_info_id).'</td></tr>';
				}

				//Age

				$age = $this->_extra_field->displayValueMobile($extra_section,7,$billingaddresses->users_info_id);

				if($age!=""){
					$billadd .= '<tr><td>Age<td> : ' .$this->_extra_field->displayValueMobile($extra_section,7,$billingaddresses->users_info_id).'</td></tr>';
				}

				//Gender

				$gender = $this->_extra_field->displayValueMobile($extra_section,9,$billingaddresses->users_info_id);

				if($gender!=""){
					$billadd .= '<tr><td>Gender<td> : ' .$this->_extra_field->displayValueMobile($extra_section,9,$billingaddresses->users_info_id).'</td></tr>';
				}

				if ($billingaddresses->phone != "") {
					$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_PHONE" ) . '  </td><td> : ' . $billingaddresses->phone . '</td></tr>';
				}
		
				$mobile = $this->_extra_field->displayValueMobile($extra_section,8,$billingaddresses->users_info_id);

				if($mobile!=""){
					$billadd .= '<tr><td>Mobile Phone<td> : ' .$this->_extra_field->displayValueMobile($extra_section,8,$billingaddresses->users_info_id).'</td></tr>';
				}

				if ($billingaddresses->user_email != "") {
					$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_EMAIL" ) . ' </td><td> : ' . $billingaddresses->user_email. '</td></tr>';
				}
				else if($user->email!='')
				{
					$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_EMAIL" ) . ' </td><td> : ' .  $user->email . '</td></tr>';
				}
				if ($billingaddresses->is_company == 1)
				{
					if ($billingaddresses->vat_number != "")
					{
						$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_VAT_NUMBER" ).' : </td><td>'.$billingaddresses->vat_number.'</td></tr>';
					}
					if ($billingaddresses->ean_number != "")
					{
						$billadd .= '<tr><td>'.JText::_("COM_REDSHOP_EAN_NUMBER" ).' :</td><td> '.$billingaddresses->ean_number.'</td></tr>';
					}
//					if ($billingaddresses->requisition_number != "")
//					{
//						$billadd .= JText::_("COM_REDSHOP_REQUISITION_NUMBER" ).' : '.$billingaddresses->requisition_number.'<br />';
//					}
					if(SHOW_TAX_EXEMPT_INFRONT)
					{
						$billadd .= '<tr><td colspan=2>'.JText::_ ("COM_REDSHOP_TAX_EXEMPT").' : ';
						if ($billingaddresses->tax_exempt == 1 ) {
							$taxexe = JText::_("COM_REDSHOP_YES" );
						} else {
							$taxexe = JText::_("COM_REDSHOP_NO" );
						}
						$billadd .= $taxexe.'</td></tr>';

						$billadd .= '<tr><td colspan=2>'.JText::_ ("COM_REDSHOP_USER_TAX_EXEMPT_REQUEST_LBL").' : ';
						if ($billingaddresses->requesting_tax_exempt == 1) {
							$taxexereq = JText::_("COM_REDSHOP_YES" );
						} else {
							$taxexereq = JText::_("COM_REDSHOP_NO" );
						}
						$billadd .= $taxexereq.'</td></tr>';
					}
				}
	...
	function replaceShippingAddress($data,$shippingaddresses)
	{
		if(strstr($data,'{shipping_address_start}') && strstr($data,'{shipping_address_end}'))
		{
			...
			if(isset($shippingaddresses) && SHIPPING_METHOD_ENABLE)
			{
				...
				/*
				$s_mobile = $this->_extra_field->displayValueMobile($extra_section,11,$shippingaddresses->users_info_id);

				if ($s_mobile != "") {
					$shippingdata=str_replace("{mobile}",$s_mobile,$shippingdata);
					$shippingdata=str_replace("{mobile_lbl}","Mobile Phone",$shippingdata);
				}

				$s_email = $this->_extra_field->displayValueMobile($extra_section,5,$shippingaddresses->users_info_id);

				if ($s_email != "") {
					$shippingdata=str_replace("{email}",$s_email,$shippingdata);
					$shippingdata=str_replace("{email_lbl}","E-mail",$shippingdata);
				}*/
			...
			$shippingdata=str_replace("{mobile}","",$shippingdata);
			$shippingdata=str_replace("{mobile_lbl}","",$shippingdata);
			$shippingdata=str_replace("{email}","",$shippingdata);
			$shippingdata=str_replace("{email_lbl}","",$shippingdata);
		...
		{
			$shipadd = '<table style="width:300px;background: none repeat scroll 0 0;">';
			if(isset($shippingaddresses) && SHIPPING_METHOD_ENABLE)
			{
			    if ($shippingaddresses->is_company == 1 && $shippingaddresses->company_name != "")
				{
					$shipadd .= '<tr><td>' .JText::_('COM_REDSHOP_COMPANY_NAME').' </td><td>:'.$shippingaddresses->company_name.'</td></tr>';
				}
				if ($shippingaddresses->firstname != "") {
					$shipadd .= '<tr><td width="86px">' .JText::_("COM_REDSHOP_FIRSTNAME" ) . ' </td><td>: ' . $shippingaddresses->firstname.'</td></tr>' ;
				}
				if ($shippingaddresses->lastname != "") {
					$shipadd .= '<tr><td>' .JText::_("COM_REDSHOP_LASTNAME" ) . ' </td><td>: ' . $shippingaddresses->lastname . '</td></tr>';
				}
				if ($shippingaddresses->address != "") {
					$shipadd .= '<tr><td>' .JText::_("COM_REDSHOP_ADDRESS" ) . ' </td><td>: ' . $shippingaddresses->address . '</td></tr>';
				}

				$cname = $this->_order_functions->getCountryName( $shippingaddresses->country_code );
				if ($cname != "") {
					$shipadd .= '<tr><td>' .JText::_("COM_REDSHOP_COUNTRY" ) . ' </td><td>: ' . JText::_( $cname) .'</td></tr>';
				}
				$sname = $this->_order_functions->getStateName( $shippingaddresses->state_code, $shippingaddresses->country_code );
				if ($sname != "") {
					$shipadd .= '<tr><td>' .JText::_("COM_REDSHOP_STATE" ) . ' </td><td>: ' . $sname. '</td></tr>';
				}

				if ($shippingaddresses->city != "") {
					$shipadd .=  '<tr><td>' .JText::_("COM_REDSHOP_CITY" ) . ' </td><td>: ' . $shippingaddresses->city . '</td></tr>';
				}

				if ($shippingaddresses->zipcode != "") {
					$shipadd .= '<tr><td>' .JText::_("COM_REDSHOP_ZIP" ) . ' </td><td>: ' . $shippingaddresses->zipcode . '</td></tr>';
				}

				if ($shippingaddresses->phone != "") {
					$shipadd .= '<tr><td>' .JText::_("COM_REDSHOP_PHONE" ) . ' </td><td>: ' . $shippingaddresses->phone .'</td></tr>';
				}

				$smobile = $this->_extra_field->displayValueMobile(14,11,$shippingaddresses->users_info_id);

				if($smobile !=""){
					$shipadd .= '<tr><td>Mobile Phone<td> : ' .$smobile.'</td></tr>';
				}

				$semail = $this->_extra_field->displayValueMobile(14,5,$shippingaddresses->users_info_id);

				if($semail !=""){
					$shipadd .= '<tr><td>E-mail<td> : ' .$semail.'</td></tr>';
				}
				...
				if($shippingaddresses->is_company==1)
				{
					...
					$shipadd.='<tr><td>' .$this->_extra_field->list_all_field_display(15,$shippingaddresses->users_info_id,1);
				}else
				{
					# additional functionality - more flexible way
					$data = $this->_extraFieldFront->extra_field_display(14,$shippingaddresses->users_info_id,"",$data);

					//$shipadd .='<tr><td>' .$this->_extra_field->list_all_field_display(14,$shippingaddresses->users_info_id,11).'</td></tr>';
	...
	function replaceShippingMethod($row=array(),$data="")
	{
		...
		{*/
//			$shipping_start = explode("{if shipping}", $data);
//			$shipping_end = explode("{shipping end if}", $shipping_start[1]);
//			$data=$shipping_start[0].$shipping_end[0].$shipping_end[1];
...
//				$data=$shipping_start[0].$shipping_end[1];
...
//		}
	...
	function replaceCartItem($data,$cart=array(),$replace_button,$quotation_mode=0)
	{
		...
		for( $i=0 ; $i < $idx; $i++ )
		{
			if(isset($cart[$i]['giftcard_id']) && $cart[$i]['giftcard_id'])
			{
				...
				$giftcard_image ="<div  class='giftcard_image'><img src='".$url."/components/com_redshop/helpers/thumb.php?filename=giftcard/".$giftcardData->giftcard_image."&newxsize=".CART_THUMB_WIDTH."&newysize=".CART_THUMB_HEIGHT."&swap=".USE_IMAGE_SIZE_SWAPPING."'></div>";
			...
			else
			{
				...
				else if(is_file(REDSHOP_FRONT_IMAGES_RELPATH."mergeImages/".$cart[$i]['attributeImage']) && isset($cart[$i]['attributeImage']))
				{
				...
				//End
				$cart_mdata = str_replace ( "{stock_product}", $cart[$i]['stock_product'], $cart_mdata );
				...
				if($replace_button)
				{
					...
					if($mainview=='cart')
					{
						$attchange = JRoute::_( 'index.php?option='.$option.'&view=product&pid='. $product_id.'&Itemid='.$Itemid);
						$update_attribute = '<a href="'.$attchange.'">'.JText::_('COM_REDSHOP_CHANGE_ATTRIBUTE').'</a>';
					}
					if($cart_attribute!="")
					...
					/*$update_cart_none ='<form style="padding:0px;margin:0px;" name="update_cart'.$i.'" method="POST">
									<label>'.$quantity.'</label>
									<input class="inputbox" type="hidden" value="'.$quantity.'" name="quantity" id="quantitybox'.$i.'" size="3">
								...
								$update_cart .='<input class="inputbox" type="text" value="'.$quantity.'" name="quantity" id="quantitybox'.$i.'" size="'.DEFAULT_QUANTITY.'" maxlength="'.DEFAULT_QUANTITY.'" onkeypress="return isNumberKey(event)" onchange="validateInputNumber(this.id);">';
	...
	function calculation($cart,$shipping=0,$user_id=0)
	{
    	...
    	$tmparr['tax'] = $vat;
		$shippingVat = 0;
		if (SHOW_SHIPPING_IN_CART && SHIPPING_METHOD_ENABLE) // if SHOW_SHIPPING_IN_CART set to no, make shipping Zero
		{
			if(!$user_id)
			{
				$user = JFactory::getUser();
	...
	function GetCartModuleCalc($redArray)
	{
		...
		$discount_total = $redArray['coupon_discount'] + $redArray['voucher_discount'] + $redArray['cart_discount'];//$redArray['discount'] + $redArray['discount2'] ;
...
//			if(strstr($cart_data,"{if shipping}") && strstr($cart_data,"{shipping end if}"))
//			{
//				$shipping_start = explode("{if shipping}", $cart_data);
//				$shipping_end = explode("{shipping end if}", $shipping_start[1]);
//				$cart_data=$shipping_start[0].$shipping_end[0].$shipping_end[1];
				$rep = true;
				if (!$checkout) {
...
//					$cart_data = $shipping_start[0].$shipping_end[1];
...
//			}
		...
		if(!APPLY_VAT_ON_DISCOUNT)	$total_for_discount = $subtotal_excl_vat;
		else $total_for_discount = $subtotal;
	...
	function replaceOrderTemplate($row,$ReceiptTemplate)
	{
		...
		if(!APPLY_VAT_ON_DISCOUNT)	$total_for_discount = $subtotal_excl_vat;
		else $total_for_discount = $row->order_subtotal;
		...
		for($d=0;$d<count($arr_discount);$d++)
		{
			if($arr_discount[$d])
			{
				...
				if($arr_discount_type[0] == 'c')
					$discount_type.= JText::_('COUPON_CODE'). ' : '. $arr_discount_type[1].'<br>';
				if($arr_discount_type[0] == 'v')
					$discount_type.= JText::_('VOUCHER_CODE'). ' : '. $arr_discount_type[1].'<br>';
			}
		}
		...
		{
			$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod_detail->element.DS.$paymentmethod_detail->element.'.xml';
...
//			$orderdetailurl 	= JURI::root ().'index.php?option=com_redshop&view=order_detail&oid=' . $order_id . '&encr=' . $row->encr_key;
	...
	function GetCartParameters()
	{
		...
		for($i=0;$i<count($allparams);$i++)
		{
			...
			if(!empty($cart_param)){
				if(strstr($cart_param[0],'cart_output') || strstr($cart_param[0],'show_with_shipping') || strstr($cart_param[0],'show_with_discount')
			   		|| strstr($cart_param[0],'show_with_vat') || strstr($cart_param[0],'show_shipping_line')){
				  	$cartparamArr[trim($cart_param[0])] = trim($cart_param[1]);
			 	}
			}
		}//echo '<pre/>';print_r($cartparamArr);exit;
		return $cartparamArr;
	}
		...
		$cartArr['user_id']=$user_id;
		 ...
		 for( $i=0 ; $i< $idx ; $i++ )
		 {
			if(!isset($cartArr[$i]['giftcard_id']) || (isset($cartArr[$i]['giftcard_id']) && $cartArr[$i]['giftcard_id']<=0))
			{
				...
				$getproprice = $retAttArr[1]; // product + attribute (price)
				$getprotax = $retAttArr[2]; // product + attribute (VAT)
				...
				$getaccprice = $retAccArr[1]; // accessory + attribute (price)
				$getacctax = $retAccArr[2]; // accessory + attribute (VAT)
	...
	function getGLSLocation($users_info_id,$classname,$shop_id=0)
	{
	   	...
	   	if($isEnabled && $classname=='default_shipping_GLS')
		{
			...
			/*if(count($ShopRespons)>0)
				$output						= '<fieldset><legend><b>'.JText::_( 'PACKAGE_COLLETCTION_SHOP' ).'</b></legend><div>';
*/
			$shopList = array();
			...
			for($i=0;$i<count($ShopRespons);$i++)
			{
				...
				/*$output .= "<div><input type='radio' $checked id='shop_id_". $ShopRespons[$i]->Number."' name='shop_id' value='". $ShopRespons[$i]->shop_id ."' />"  ;
				$output .= $ShopRespons[$i]->CompanyName . ", " . $ShopRespons[$i]->Streetname.", ". $ShopRespons[$i]->ZipCode . ", ". $ShopRespons[$i]->CityName . "</div>";
				$output .= "<div>".$ShopRespons[$i]->openingTime. "</div><br>";*/
					...
					if(count($rate)>0)
					{
						...
						if($template_rate_middle!="")
						{
							...
							for($i=0;$i<count($rate);$i++)
							{
								...
								$shipping_rate_name = '<input type="radio" name="shipping_rate_id" value="'.$rate[$i]->value.'" '.$checked.' onclick="javascript:onestepCheckoutProcess(this.name,\''.$classname.'\');">'.html_entity_decode($rate[$i]->text);
	...
	function replaceCreditCardInformation($payment_method_id=0)
	{
		...
		$montharr[]   = JHTML::_('select.option', '0',JText::_('COM_REDSHOP_MONTH'));
	  	$montharr[]   = JHTML::_('select.option', '01',1);//JText::_('COM_REDSHOP_JAN'));
	  	$montharr[]   = JHTML::_('select.option', '02',2);//JText::_('COM_REDSHOP_FEB'));
	  	$montharr[]   = JHTML::_('select.option', '03',3);//JText::_('COM_REDSHOP_MAR'));
	  	$montharr[]   = JHTML::_('select.option', '04',4);//JText::_('COM_REDSHOP_APR'));
	  	$montharr[]   = JHTML::_('select.option', '05',5);//JText::_('COM_REDSHOP_MAY'));
	  	$montharr[]   = JHTML::_('select.option', '06',6);//JText::_('COM_REDSHOP_JUN'));
	  	$montharr[]   = JHTML::_('select.option', '07',7);//JText::_('COM_REDSHOP_JUL'));
	  	$montharr[]   = JHTML::_('select.option', '08',8);//JText::_('COM_REDSHOP_AUG'));
	  	$montharr[]   = JHTML::_('select.option', '09',9);//JText::_('COM_REDSHOP_SEp'));
	  	$montharr[]   = JHTML::_('select.option', '10',10);//JText::_('COM_REDSHOP_OCT'));
	  	$montharr[]   = JHTML::_('select.option', '11',11);//JText::_('COM_REDSHOP_NOV'));
	  	$montharr[]   = JHTML::_('select.option', '12',12);//JText::_('COM_REDSHOP_DEC'));
		...
		{
//			$paymentpath=JPATH_SITE.DS.'plugins'.DS.'redshop_payment'.DS.$paymentmethod->element.'.php';
...
//				$cardinfo .='<tr valign="top"><td align="right" nowrap="nowrap" width="10%"><label for="order_total">'.JText::_('COM_REDSHOP_ORDERTOTAL' ).'</label></td>';
//				$cardinfo .='<td>'.$this->_producthelper->getProductFormattedPrice($order->order_total).'</td></tr>';
	...
	//						$style = ($checked!="") ? "" : "none";
		...
		if(count($paymentmethod)==1 && $is_creditcard=="0")
		{
			$template_desc = "<div style='display:none;'>".$template_desc."</div>";
		}
	...
	function replaceNewsletterSubscription($template_desc="",$onchange=0)
	{
		if(strstr($template_desc,"{newsletter_signup_chk}"))
		{
			...
			if (DEFAULT_NEWSLETTER != 0)
			{
				...
				/*if($subscribe)
				{
					if(strstr($template_desc,"{newsletter_unsubscribe}"))
					{
						if($onchange)
						{
							//$link = " onchange='window.location.href=\"".JRoute::_('index.php?option=com_redshop&view=account&task=newsletterUnsubscribe&Itemid='.$Itemid,false)."\"; '";
							$link = " onchange='window.location.href=\"".JUri::root()."index.php?option=com_redshop&view=account&task=newsletterUnsubscribe&tmpl=component&Itemid=".$Itemid."\"";

						}
						//$newslettersignup = "<input type='checkbox' name='newsletter_signoff' value='1' ".$link.">";
						$newslettersignup = "<input type='checkbox' name='newsletter_signoff' value='1' '$link'>";
						$newslettersignup_lbl = JText::_('COM_REDSHOP_SIGN_OFF_FOR_NEWSLETTER' );
					}
				}*/
				if($subscribe==0)
				{
					if($onchange)
					{
						//$link = " onchange='window.location.href=\"".JRoute::_('index.php?option=com_redshop&view=account&task=newsletterSubscribe&Itemid='.$Itemid,false)."\"; '";
...
//				}else{
//					$pSubtotal = $cart['product_subtotal_excl_vat'];
//				}
				$tmpsubtotal = $pSubtotal;
				if($view =='cart')
					$tmpsubtotal = $pSubtotal  - $cart['voucher_discount'] - $cart['cart_discount'] ;// - $cart['coupon_discount'];

		 		if($dis_type==0)
		 		{
		 			$couponValue = $coupon->coupon_value;
		 			if($couponValue>$pSubtotal)
		 				$couponValue=$pSubtotal;
		 		} else {
...
//				else
//					$product_price = $productArr['product_price_excl_vat'];
				...
				$totalDiscount = $cart['voucher_discount'] + $cart['cart_discount'] + $cart['coupon_discount'] ;//+ $voucherValue;
				$tmpsubtotal = $product_price  - $cart['coupon_discount'] - $cart['cart_discount'] ;// - $cart['voucher_discount'];
	...
	function globalvoucher($voucher_code)
	{
		...
		$query = "SELECT product_id,v.* from ".$this->_table_prefix."product_voucher_xref as pv  "
				 //. $this->_table_prefix."product_voucher as v "
	...
	function checkQuantityInStock($data=array(),$newquantity=1,$minQuantity=0)
	{
		...
		if(USE_STOCKROOM==1)
		{
			if(($product_preorder == "global" && !ALLOW_PRE_ORDER) || ($product_preorder == "no") || ($product_preorder == "" && !ALLOW_PRE_ORDER))//$newquantity!=$data['quantity']
			{
			...
			if(($product_preorder == "global" && ALLOW_PRE_ORDER) || ($product_preorder == "yes") || ($product_preorder == "" && ALLOW_PRE_ORDER))
			{
				 ...
				 $preorder_currentStock = $stockroomhelper->getPreorderStockroomTotalAmount($data['product_id']);//die();
				...
				if(count($attArr)<=0)
				{
		       	...
		       	if($newquantity >= 0)//($newquantity!=$data['quantity'])
				{
		...
		for($i=0;$i<count($attArr);$i++)
		{
			...
			for($k=0;$k<count($propArr);$k++)
			{
				if(USE_STOCKROOM==1)
				{
					...
					if(($product_preorder == "global" && ALLOW_PRE_ORDER) || ($product_preorder == "yes") || ($product_preorder == "" && ALLOW_PRE_ORDER))//$newquantity!=$data['quantity']
						{
						 	$regular_property_stock = $stockroomhelper->getStockroomTotalAmount($propArr[$k]['property_id'],"property");//die();
							$Preorder_property_stock = $stockroomhelper->getPreorderStockroomTotalAmount($propArr[$k]['property_id'],"property");
							$property_stock = $regular_property_stock + $Preorder_property_stock;//die();

					  	}
					$ownreserveStock = $stockroomhelper->getCurrentUserReservedStock($propArr[$k]['property_id'],"property");
			       	if($property_stock >= 0)//if($property_stock > 0)
			       	{
				...
				for($l=0;$l<count($subpropArr);$l++)
				{
					if(USE_STOCKROOM==1)
					{
							...
							if(($product_preorder == "global" && ALLOW_PRE_ORDER) || ($product_preorder == "yes") || ($product_preorder == "" && ALLOW_PRE_ORDER))
							{
								...
								$ownreserveStock = $stockroomhelper->getCurrentUserReservedStock($propArr[$k]['property_id'],"property");
						       	if($subproperty_stock >= 0)//if($subproperty_stock > 0)
						       	{
	 ...
	 * @param $cart_id - #__redshop_usercart table key id
		...
		if(count($cart_items)>0)
		{
			...
			for($i=0;$i<count($cart_items);$i++)
			{
				...
				if(!$isStock)
				{
					...
					#continue;	// if product is out of stock don't add to cart - Note: if require remove comment
				}
				...
				if(count($req_attribute)>0)
				{
					...
					$msg = $requied_attribute_name." ".JText::_('COM_REDSHOP_IS_REQUIRED'); // give error as first attribute is required
					$setCartItem = false;
				}
	...
	function addProductToCart($data=array())
	{
		...
		if(!AJAX_CART_BOX)
		{
			...
			if($fieldreq!="")
			{
//				JError::raiseWarning(21, $fieldreq);
//				return false;
				return $fieldreq;
			}
		}
		...
		if(!empty($selectProp[0]))
		{
			...
			$attributeImage	= implode('_p',array($product_id,''));
			$attributeImage	.= implode('_p',$selectProp[0]);
...
//				return false;
		...
		if(!empty($discountArr))
		{
			...
			$data['product_price'] += $discountArr[2] +$discountArr[3];	// calculator price without VAT
		...
		if(!INDIVIDUAL_ADD_TO_CART_ENABLE)
		{
			...
			if(count($attribute_template)>0)
			{
				...
				if(count($req_attribute)>0)
				{
					...
					$requied_attribute_name = implode(", ",$requied_attributeArr);
					$msg = urldecode($requied_attribute_name)." ".JText::_('COM_REDSHOP_IS_REQUIRED'); // give error as first attribute is required
...
//					return false;
				...
				if(count($req_property)>0)
				{
					...
					$msg = urldecode($requied_subattribute_name)." ".JText::_('COM_REDSHOP_SUBATTRIBUTE_IS_REQUIRED'); // give error as second attribute is required
...
//					return false;
		...
		/****************ADD WRAPPER PRICE*******************/
   	  ...
   	  // 	$cart[$idx]['preorder_value']=$data['preorder_value'.$data['product_id']];
		...
		for( $i=0 ; $i< $idx ; $i++ )
		{
		   	if($cart[$i]['product_id'] == $data['product_id'])
		   	{
				 ...
				 *
				 *  For future enhansment - not working enymore
				 */
				JPluginHelper::importPlugin('redshop_product');
...
//				return false;
	...
	function discountCalculatorData($product_data,$data)
	{
   	 	...
   	 	if($use_discount_calculator)
		{
   	 		...
   	 		if($calculator_price)
			{
				...
				$calc_output .= "<br />".$discount_cal['pdcextra_data']; # extra selected value data
				$calc_output_array['calcextra_ids'] = $discount_cal['pdcextra_ids']; # extra selected value ids
	...
	function discountCalculator($get)
	{
		...
		if(count($discount_calc_data)){
			...
			if($use_range){
				...
				//$display_final_area = $finalArea;

				$price_per_piece = $area_price*$finalArea;
				//$price_per_piece = $area_price*$display_final_area;
				/**
				 * changed by Gunjan - but confusing :(
				 */
				$price_per_piece = $area_price;


				// applying TAX
	     	 	//$area_price += $this->_producthelper->getProductTax($product_id,$area_price);
	...
	function getProductStock($product_id=0, $identity)
	{
			$and = "";
			$list = "";
		if($identity == 'prod'){
			...
			$query = "SELECT quantity FROM `jml_redshop_product_stockroom_xref` "
					."WHERE 1=1 "
					.$and
					."ORDER BY quantity ";
			$this->_db->setQuery($query);
			$list = $this->_db->loadObjectlist();
			if($list[0]->quantity > 30){		
		...
		if($identity == 'attri'){
			if($product_id)
			{
				$and .="AND section_id = '".$product_id."' ";
			}
			
			$query = "SELECT quantity FROM `jml_redshop_product_attribute_stockroom_xref` "
					."WHERE 1=1 "
					.$and
					."ORDER BY section_id ";
			$this->_db->setQuery($query);
