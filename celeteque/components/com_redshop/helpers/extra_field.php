 ...
 */
	...
	function list_all_field($field_section="",$section_id=0,$uclass='')
	{
		...
		for($i=0;$i<count($row_data);$i++)
		{
			...
			$ex_field .= '<tr id="'.$row_data[$i]->field_title.'"><td style="font-size: 11px;" width="100" valign="top" align="left"><label>'.JText::_($row_data[$i]->field_title).'</label></td><td style="font-size: 11px;"> : ';
			...
			switch ($type)
			{
				case 1: // 1 :- Text Field
						...
						$ex_field .='<input '.$class.' type="text" maxlength="'.$row_data[$i]->field_maxlength.'" name="'.$row_data[$i]->field_name.'" id="'.$row_data[$i]->field_name.'" value="'.$text_value.'" size="31" />';			        				break;
				case 2: // 2 :- Text Area
			 			$textarea_value='';
			...
			if($type=="12")
			{
				$ex_field .= '</td></tr><tr><td></td><td align="left" style="font-size: 8px; color: blue; padding-left:30px;">(Day-Month-Year)</td></tr>';
			}
			else
			{
				$ex_field .= '</td></tr>';
...
//		}
			      ...
			      case 13 :
					...
					for($c=0;$c<count($document_value);$c++)
					{
						if(@in_array($document_value[$c]->value_id,$chk_data))
						{
							...
							$str_image_link =  $image_link[$document_value[$c]->value_id];//$image_link[$c];
	...
	function getFieldValueBYName($id)
	{
		$q = "SELECT * FROM ".$this->_table_prefix."fields_value "
			."WHERE field_title='".$id."' "
			."ORDER BY value_id ASC ";
		$this->_db->setQuery($q);
		$list = $this->_db->loadObjectlist();
		return $list;
